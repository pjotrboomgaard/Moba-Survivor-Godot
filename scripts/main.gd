...
## 2026-09-16 user rule: NO pixel-art effect frames on ANY ability. Every ability is
## vector-only (LightningEffect). Placed objects (turrets / wards / mines) still show
## their own pixel-art SummonEntity sprite — but that is the object itself, not a VFX
## burst, so no AbilityVfx is spawned for any ability id.
...
func _play_ability_effect(ability_id: String, effect_style: int, points: PackedVector2Array) -> void:
	...
	# (the SummonEntity spawned by _spawn_summon) already shows the pixel-art turret,
	# so a separate AbilityVfx burst was redundant and read as a stray pixel-art
	# explosion around the hero. Every ability is now vector-only.
